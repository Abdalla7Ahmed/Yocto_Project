...
// PWM 
struct pwm_device *mypwm;


#define pwm_number      0
#define pwm_label       "test_pwm"
#define ONE_Second      1000000000
#define high            500000000
...
ssize_t Driver_Write (struct file *file,const char __user *user_buffer, size_t count, loff_t *offset)
{
    ...
    retval = kstrtol_from_user(user_buffer,count -1,0,&val);
    if(retval==0)
    {
        printk("%s the value in ms is %ld\n",__FUNCTION__,val);
    }
    else
    {
    ...
    if(val > 1000)
    {
       printk("%s not valid duty cycle \n",__FUNCTION__); 
    }
    else 
    {
        pwm_config(mypwm,val*1000000,ONE_Second);
...
static int __init Module_init(void)
{
    ...
    mypwm = pwm_request(pwm_number,pwm_label);
    if(mypwm == NULL)
    {
        printk("Failed to request pwm%d\n", pwm_number);
        goto ERROR_PWM_REQUEST;
    }
    else 
    {
       printk("pwm request done\n"); 
    }

    pwm_config(mypwm,high,ONE_Second);
    pwm_enable(mypwm);
